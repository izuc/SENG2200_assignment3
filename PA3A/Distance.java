/**
* The Distance class extends Event and implements the Comparable<Distance> interface in order to have the compareTo method implemented.
* The attempts are stored as a two-dimensional array that has the attempt distance and also the attempt number. It has a method
...
* The compareTo implementation compares two Distance objects, which both fetch the sorted
...
* the arrays comparing when the attempt was done until a Distance is found to be better than the other.
* @author Lance Baker (c3128034)
*/
public class Distance extends Event implements Comparable<Distance> {
	...
	public Distance(String name, String country) {
		super(name, country);
	...
	*/
	...
	/**
	* The setData method receives the distance, and it will traverse the array until a not entered
	...
	*/
	public int compareTo(Distance event) {
	...
	*/
	public boolean lessThan(Event event) throws OlympicEventException  {
		if (event instanceof Distance) {
			return (this.compareTo((Distance)event) >=0);
		} else {
