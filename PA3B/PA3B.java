...
/**
* The PA3B class is the main class (which is static) in the application. It contains the main method (which invokes the other 
...
public class PA3B {
	...
	/**
	* The factory method receives the dataType number, the athlete's name and their country code.
	* It then will instantiate and return an Event instance corresponding to the dataType int given. 
	* Otherwise, if the value is not there it will return null.
	*/
	private static Event factory(int dataType, String name, String country) {
		return ((dataType == 1) ? new Distance(name, country) : 
			    (dataType == 2) ? new Timed(name, country) : null);
	}
	...
	*/
	private static SortedEventList sortEvents(LinkedList<Event> events) throws OlympicEventException {
	...
	*/
	private static SortedEventList fetchEvents(BufferedReader br) throws Exception {
		...
		int dataType = Integer.parseInt(br.readLine().trim());
		// A simple selection to determine how many attempts are for each Event.
		int noAttempts = (dataType == 1) ? 6 : (dataType == 2) ? 1 : 0;
		// The count of atheletes.
		...
		for (int i = 0; i < records; i++) {
			...
			Event event = factory(dataType, values[0], values[1]);
			if (event != null) {
				events.append(event);
			}
		}
		
		for (int i = 0; i < noAttempts; i++) { // Then iterate 6 times.
			for (Event event : events) { // It will iterate through each Event object in the LinkedList.
				...
				event.setData(Double.parseDouble(data));
			}
		}
